@extends('admin.layouts')

@section('title', "$opportunity->title | Edit opportunity detail")

@section('styles')
<link href="{{ Vite::asset('resources/assets/vendors/flatpickr/flatpickr.min.css')}}" rel="stylesheet">
@endsection

@section('content')
@foreach($errors->all() as $error)
    <li class="alert alert-danger">{{ $error }}</li>
@endforeach
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    <h4>Edit Opportunity</h4>
                </div>
                <nav aria-label="breadcrumb" class="mt-3">
                    <ol class="breadcrumb">
                      <li class="breadcrumb-item"><a href="{{ route('admin.opportunity.index') }}">Opportunities</a></li>
                      <li class="breadcrumb-item"><a href="{{ route('admin.opportunity.show', $opportunity->id) }}">{{ $opportunity->title }}</a></li>
                      <li class="breadcrumb-item active" aria-current="page">Edit</li>
                    </ol>
                  </nav>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.opportunity.update', $opportunity->id) }}">
                  @csrf
                  <input type="hidden" name="_method" value="PATCH" />
                    <div class="row">
                        <div class="mb-3 col-6">

                            <label class="form-label" for="basic-form-name">Prospect Name</label>
  
                            <input class="form-control" name="prospect_name" value="{{ $opportunity->prospect_name }}" id="basic-form-name" type="text" placeholder="Prospect Name" />
                          </div>
                          <div class="mb-3 col-6">
  
                            <label class="form-label" for="basic-form-email">Prospect Email</label>
  
                            <input class="form-control" name="prospect_email" value="{{ $opportunity->prospect_email }}" id="basic-form-email" type="email" placeholder="name@example.com" />
                          </div>
                          <div class="mb-3 col-6">
  
                            <label class="form-label" for="basic-form-password">Prospect Phone</label>
  
                            <input class="form-control" name="prospect_phone" value="{{ $opportunity->prospect_phone }}" id="basic-form-password" type="text" placeholder="Prospect Phone" />
                          </div>
                          <div class="mb-3 col-6">
                            <label class="form-label" for="basic-form-password">Estimated Budget</label>
                            <div class="input-group">
                              <span class="input-group-text" id="basic-addon1">$</span>
                            
                              <input class="form-control" name="estimated_budget" value="{{ $opportunity->estimated_budget }}" type="number" min="10" placeholder="Estimated Budget ($)" aria-label="est_budget" aria-describedby="basic-addon1" />
                            </div>
                          </div>

                          <div class="mb-3 col-6">
  
                            <label class="form-label" for="basic-form-email">Title</label>
  
                            <input class="form-control" required name="title" value="{{ $opportunity->title }}" id="basic-form-title" type="text" placeholder="Title" />
                          </div>

                          <div class="mb-3 col-6">
  
                            <label class="form-label" for="basic-form-location">Location</label>
  
                            <input class="form-control" name="location" value="{{ $opportunity->location }}" id="basic-form-location" type="text" placeholder="Location" />
                          </div>

                          <div class="mb-3 col-6">
                            <label class="form-label" for="datepicker">Closing Date</label>

                            <input class="form-control" name="closing_date" value="{{ Carbon\Carbon::parse($opportunity->closing_date)->format('Y-m-d') }}" id="datepicker" type="date" placeholder="dd/mm/yyyy"/>
                            
                          </div>
                          
                          <div class="mb-3 col-6">
  
                            <label class="form-label" for="basic-form-type">Opportunity Type</label>
  
                            <select name="opportunity_type" class="form-select" id="basic-form-gender" aria-label="Default select example">
                              <option selected disabled>Select opportunity type</option>
                              <option value="prospect-intelligence" {{ ($opportunity->opportunity_type == 'prospect-intelligence') ? 'selected' : '' }}>Prospect Intelligence</option>
                              <option value="request-for-proposal" {{ ($opportunity->opportunity_type == 'request-for-proposal') ? 'selected' : '' }}>Request For Proposal</option>
                              <option value="buying-signal" {{ ($opportunity->opportunity_type == 'buying-signal') ? 'selected' : '' }}>Buying Signal</option>
                            </select>
                          </div>
                          <div class="mb-3">
  
                            <label class="form-label" for="basic-form-info">Opportunity Info</label>
  
                            <textarea class="form-control" name="opportunity_info" id="basic-form-info" rows="5" placeholder="Opportunity Info">{{ $opportunity->opportunity_info }}</textarea>
                          </div>
  
                          <button class="btn btn-primary" type="submit">Update</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection

@section('scripts')
<script src="{{ Vite::asset('resources/assets/vendors/flatpickr/flatpickr.min.js') }}"></script>
@endsection